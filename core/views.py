...
def sort(request):
    ...
    selected_movies=MovieFilter.objects.filter(genres=genres).all().order_by(cur)
    if new_order=='dsc':
        selected_movies=selected_movies.reverse()
    pagesize=10
    paginator=Paginator(selected_movies, pagesize)
    ...
    if len(selected_movies)<=pagesize:
        context={
        'movies':movies,
        'multipage':False,
        'order':new_order,
...
def index(request):
    if request.method =='POST':
        ...
        ls=["Comedy","ScienceFiction","Romance","Drama","Horror","Thriller","Action","Adventure","Family",
            "Western","Animation","Documentary","TVMovie","History","War","Crime","Fantasy","Mystery"]
        all_movies=Movie.objects.all()
        return render(request,'list.html',{'all':all_movies,'ls':ls})
